...
library(knitr)
opts_chunk$set(echo=FALSE, fig.height=9, fig.width=9, message=FALSE, warning=FALSE)
...
```{r hierarchical_clustering, fig.height=9, fig.width=9}
...
```{r dendrogram, fig.height=9, fig.width=9}
clustering <- hclust(dist(scale_matrix_ox), "ward.D2")
clustering$labels <- gsub(" College", "", clustering$labels)
...
```{r correlation_matrix, fig.height=9, fig.width=9}
# impute missing values using the mean
scale_matrix_ox[is.na(scale_matrix_ox)] <- 0

# View correlation matrix
aheatmap(cor(scale_matrix_ox), breaks = seq(-1, 1, 0.02), cexCol = 1.0, cexRow = 0.25, treeheight = c(0, 30))
```
...
```{r PCA, fig.height=9, fig.width=9}
# compute PCA
...
ggplot(ox_pca_dt, aes(PC1, PC2, colour = Year_of_Foundation)) +
  geom_point() +
  geom_label_repel(aes(label = Name), size = 3, force = 4) +
...
```{r compare_dendrograms, fig.height=9, fig.width=9}
# recluster with pc's
clustering_pca <- hclust(dist(ox_pca$x[, 1:9]), "ward.D2")
clustering_pca$labels <- gsub(" College", "", clustering_pca$labels)
...
```{r original_norrington, fig.height=9, fig.width=9}
# Data from https://www.ox.ac.uk/about/facts-and-figures/undergraduate-degree-classifications
# Load data
...
```{r new_norrington, fig.height=9, fig.width=9}
# create plot
